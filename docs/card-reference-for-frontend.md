...
3. **Never show a number without its provenance.** Every panel estimate carries `formula`,
   ...
   wherever `panel.value` appears. Show `panel.kind` there too, as a label: a
   `planning_estimate` (a VA planning anchor, awaiting its primary source), a
   `modeled_estimate` (CDC PLACES × VetPop) and a `measured` count (emPOWER Medicare proxy)
   are different kinds of number and must never read alike.
...
Per-card facts, generated from `cards/*.yaml` (`make card-docs`; a test fails if this table
is stale):

<!-- card-facts:begin (generated by `make card-docs`; do not edit) -->
| # | `card_id` | version | window | evidence | care team (pre / during) | patient sentences | escalation (emergency) | claims | caveats | sources |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
- **Note for the UI:** the patient text opens with "Your medicine helps you stay well —
  keep taking it, even in a heat wave." Do not let a layout truncate that sentence.
...
- **The only card with `safety.do_not_stop_medication: false`**, so `safety_message` is
  ...
  Handle the null.
- **The patient text contains two phone numbers**: the KHARES national kidney emergency
  line (866-446-3507) and the older KCER number (1-866-901-3773), kept until the legacy line
  is confirmed retired. Render both as tappable `tel:` links; the server-rendered pages do
  this with the `tel_links` filter, which wraps the digits and leaves the text unchanged.

...
| `states.geojson` | `scripts/build_state_boundaries.py` | Census cartographic boundary shapefile, states, **1:5m**, vintage 2023 (`cb_2023_us_state_5m.zip`, retrieved 2026-09-21; raw zip in `raw/`) — same scale and vintage as the counties, so borders coincide. 56 states and territories; coordinates rounded to 4 decimals. Map overlay only (outlines drawn above the county fill). | built |
| `countries.geojson` | `scripts/build_countries.py` | Natural Earth **1:50m** Admin 0 – Countries (`ne_50m_admin_0_countries.zip`, https://naciscdn.org/naturalearth/50m/cultural/, public domain, retrieved 2026-09-22; raw zip in `raw/`). Canada, Mexico, Cuba and the Bahamas only; coordinates rounded to 2 decimals. Map backdrop drawn under the US counties — carries no data. | built |
...
| `eaglei_customers.csv` | `scripts/build_eaglei_customers.py` | ORNL EAGLE-I figshare deposit (doi:10.6084/m9.figshare.24237376) supplementary file `MCC.csv`: **modeled electric customers per county, 2022** (Moehl et al.; the denominator used with Brelsford et al. 2024, *Sci Data*, doi:10.1038/s41597-024-03095-5). Retrieved 2026-09-21; raw copy `raw/eaglei_MCC.csv`. 3,233 counties (FIPS zero-padded; the file's "Grand Total" row dropped). Customers are meters/accounts, not people. `outage_pct = customers_out / customers × 100` for EAGLE-I events. | built |
| `eaglei_state_coverage.csv` | `scripts/build_eaglei_customers.py` | Same deposit, `coverage_history.csv`: share of each state's customers whose utilities EAGLE-I collects, latest vintage (2022-01-01). Nationally ~92 % — the "~8 % uncovered" footnote. Raw copy `raw/eaglei_coverage_history.csv`. | built |
| `empower_county.csv`, `empower_zip.csv` | `scripts/build_empower.py` | **HHS emPOWER public REST service** (`HHS_emPOWER_REST_Service_Public`, ArcGIS Online, owner DHHS_gissupport; layer 2 county, layer 1 ZIP — ids and field names pinned, build fails on drift). De-identified monthly counts of Medicare (FFS + Advantage) beneficiaries with electricity-dependent DME claims, plus ESRD-dialysis, O₂, home-health, hospice and any-healthcare-service sub-counts and total Medicare beneficiaries. Vintage (service `dataLastEditDate`) and retrieval date are in each file's `#` header line; refresh manually (`--refresh`), monthly. 3,233 counties; 31,919 ZIPs. Small cells (1–10) are masked to 11 upstream. **A measured Medicare proxy, not veteran-specific** — always labeled so, never a replacement for a veteran denominator. Raw county JSON in `raw/`; the 8 MB raw ZIP pull is git-ignored and re-downloaded. | built |
